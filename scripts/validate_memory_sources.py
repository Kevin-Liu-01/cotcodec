...
if __package__:
    ...
    from seal_astra_native_lifecycle_evidence import (  # type: ignore[no-redef]
        AstraEvidenceError,
        validate_astra_native_lifecycle_evidence,
    )
    ...
    *,
    ...
    for name, receipt in files.items():
        ...
        if receipt.get("bytes") != len(data):
            raise MemorySourceError(f"{entry_id}: {owner} file {name} byte count drifted")
        if receipt.get("sha256") != hashlib.sha256(data).hexdigest():
            raise MemorySourceError(f"{entry_id}: {owner} file {name} SHA-256 drifted")
        decoded[name] = data
    return decoded
...
def _validate_local_evidence_bundle(
    ...
    if entry["evidence_grade"] == "local-negative-reproduced":
        if entry_id == "astra-working-set":
            ...
            except AstraEvidenceError as exc:
        ...
        if entry_id == "lightmem":
            try:
                validate_lightmem_offline_evidence(bundle, project_root=PROJECT_ROOT)
        ...
        if entry_id == "agent-recall":
            try:
                validate_agent_recall_lifecycle_evidence(bundle, project_root=PROJECT_ROOT)
            except AgentRecallEvidenceError as exc:
                raise MemorySourceError(f"{entry_id}: {exc}") from exc
            return
        if entry_id == "activegraph-event-sourced-runtime":
            try:
                validate_activegraph_lifecycle_evidence(bundle, project_root=PROJECT_ROOT)
            except ActiveGraphEvidenceError as exc:
                raise MemorySourceError(f"{entry_id}: {exc}") from exc
            return
        if entry_id == "agenticow":
            try:
                validate_agenticow_lifecycle_evidence(bundle, project_root=PROJECT_ROOT)
        ...
        if entry_id == "hermes-observational-memory":
            if (
                bundle.get("evidence_kind") != "standalone-hermes-provider-lifecycle-negative"
                or bundle.get("publication_ready") is not False
                or bundle.get("status") != "BLOCKED_NO_PROVIDER_NATIVE_DELETE_OR_ERASURE"
                ...
                raise MemorySourceError(f"{entry_id}: lifecycle negative evidence contract drifted")
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: lifecycle artifact root is unsafe")
            ...
            if not isinstance(expected_files, dict) or set(expected_files) != {
                ...
                raise MemorySourceError(f"{entry_id}: lifecycle artifact roster drifted")
            ...
            for name, expected_sha256 in expected_files.items():
                ...
                if (
                    ...
                    raise MemorySourceError(f"{entry_id}: lifecycle artifact {name} is invalid")
                data = path.read_bytes()
                if hashlib.sha256(data).hexdigest() != expected_sha256:
                    raise MemorySourceError(f"{entry_id}: lifecycle artifact {name} drifted")
            ...
            try:
                ...
                raise MemorySourceError(f"{entry_id}: lifecycle artifact JSON is invalid") from exc
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: lifecycle report semantics drifted")

            manifest_files = manifest.get("files")
            if not isinstance(manifest_files, list) or len(manifest_files) != 26:
                raise MemorySourceError(f"{entry_id}: lifecycle manifest roster drifted")
            seen: set[str] = set()
            lifecycle_root = artifact_root / "lifecycle-291"
            for row in manifest_files:
                if not isinstance(row, dict) or set(row) != {"bytes", "path", "sha256"}:
                    raise MemorySourceError(f"{entry_id}: lifecycle manifest row is invalid")
                name = row["path"]
                if (
                    not isinstance(name, str)
                    or not name
                    ...
                    raise MemorySourceError(f"{entry_id}: lifecycle manifest path is unsafe")
                seen.add(name)
                path = lifecycle_root / name
                if not path.is_file() or path.is_symlink():
                    raise MemorySourceError(f"{entry_id}: lifecycle manifest file is missing")
                data = path.read_bytes()
                if row["bytes"] != len(data) or row["sha256"] != hashlib.sha256(data).hexdigest():
                    raise MemorySourceError(f"{entry_id}: lifecycle manifest content drifted")
            if (
                manifest.get("report_sha256") != expected_files["lifecycle-291/report.json"]
                ...
                or job.get("image_archive_sha256") != bundle.get("image_archive_sha256")
                or job.get("sbom_sha256") != expected_files["sbom-291/sbom.spdx.json"]
                or job.get("gpu_inventory_sha256") != expected_files["gpu-inventory-291.txt"]
                or job.get("image_inspect_sha256") != expected_files["image-inspect-291.json"]
                or source.get("archive_sha256") != bundle.get("source_archive_sha256")
            ):
                raise MemorySourceError(f"{entry_id}: lifecycle provenance binding drifted")
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: lifecycle source archive path is unsafe")
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: lifecycle source archive drifted")
            return
        if entry_id == "supermemory":
            if (
                bundle.get("evidence_kind") != "binary-only-native-negative-reproduction"
                or bundle.get("publication_ready") is not False
                or bundle.get("run_count") != 2
                or bundle.get("claim_boundary")
                != {
            ...
            if (
                ...
                or bundle.get("execution_identity_sha256s") != verified["execution_identities"]
            ):
                raise MemorySourceError(f"{entry_id}: Supermemory evidence receipt drifted")
            return
        if (
        ...
        if entry_id == "all-mem":
            ...
            if (
                ...
                or bundle.get("observed_rank_orders") != verified["observed_rank_orders"]
            ):
                raise MemorySourceError(f"{entry_id}: All-Mem evidence receipt drifted")
            return
        if entry_id == "magic-context":
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: Magic Context evidence receipt drifted")
            return
        if entry_id == "hermes-holographic":
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: Holographic evidence receipt drifted")
            return
        if entry_id == "hermes-byterover-cli":
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: ByteRover evidence receipt drifted")
            return
        if entry_id == "openviking":
            ...
            if (
                ...
                or bundle.get("state_manifest_sha256s") != verified["state_manifest_sha256s"]
                or bundle.get("residue_file_counts") != verified["residue_file_counts"]
                or bundle.get("execution_identity_sha256s")
                != verified["execution_identity_sha256s"]
            ):
                raise MemorySourceError(f"{entry_id}: OpenViking evidence receipt drifted")
            return
        if entry_id == "hermes-hindsight-native":
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: Hindsight evidence receipt drifted")
            return
        if entry_id == "mem0-lifecycle-adapter":
            ...
            if (
                ...
                or bundle.get("stable_projection") != verified["stable_projection"]
                or bundle.get("stable_projection_sha256") != verified["stable_projection_sha256"]
                or bundle.get("report_sha256s") != verified["report_sha256s"]
                or bundle.get("source_receipt_sha256") != verified["source_receipt_sha256"]
                or bundle.get("crash_scope_plaintext_proof_roots")
                != verified["crash_scope_plaintext_proof_roots"]
            ):
                raise MemorySourceError(f"{entry_id}: Mem0 lifecycle evidence receipt drifted")
            return
        if entry_id == "graphiti-native-lifecycle-adapter":
            ...
            try:
                ...
                raise MemorySourceError(f"{entry_id}: {exc}") from exc
            if (
                bundle.get("status") != "BLOCKED_FALKORDBLITE_ARM64_MODULE_ARCHITECTURE_MISMATCH"
                or bundle.get("shared_image_id") != verified["image_id"]
                or bundle.get("manifest_root_sha256") != verified["manifest_root"]
                or bundle.get("module_architecture") != verified["module_architecture"]
                or bundle.get("execution_identity_sha256s") != verified["execution_identities"]
                or bundle.get("claim_boundary")
                != {
                    "container_lifecycle_executed": False,
                    "h100_admission": "forbidden-for-this-revision-and-runtime",
                    "reason": ("BLOCKED_FALKORDBLITE_ARM64_MODULE_ARCHITECTURE_MISMATCH"),
                }
            ):
                raise MemorySourceError(f"{entry_id}: Graphiti negative evidence receipt drifted")
            return
        if entry_id == "mnemosyne-cognitive-os":
            try:
                validate_mnemosyne_cognitive_evidence(bundle, project_root=PROJECT_ROOT)
            except MnemosyneCognitiveEvidenceError as exc:
                raise MemorySourceError(f"{entry_id}: {exc}") from exc
            return
        if entry_id == "memforge":
            try:
                validate_memforge_fresh_install_evidence(bundle, project_root=PROJECT_ROOT)
        ...
        for index, run in enumerate(runs):
            ...
            files = _validate_embedded_files(entry_id, run.get("files"), owner=f"runs[{index}]")
            try:
            ...
            if (
                ...
                or run.get("execution_identity_sha256") != verified["execution_identity_sha256"]
                or verified["image_id"] != bundle.get("shared_image_id")
                or verified["projection"] != bundle.get("deterministic_projection")
            ):
                raise MemorySourceError(f"{entry_id}: embedded negative semantic receipt drifted")
        ...
        if evidence_kind == "native-control-admission":
            ...
            files = _validate_embedded_files(entry_id, bundle.get("files"), owner="bundle")
            try:
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: provider conformance result identity drifted")
            ...
            try:
                ...
                raise MemorySourceError(f"{entry_id}: {exc}") from exc
            if (
                bundle.get("semantic_projection") != verified["semantic_projection"]
                or bundle.get("execution_state_roots") != verified["execution_state_roots"]
                or bundle.get("client_image_id") != verified["client_image_id"]
                or bundle.get("manifest_root_sha256") != verified["manifest_root"]
            ):
                raise MemorySourceError(f"{entry_id}: native lifecycle result identity drifted")
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: natural component evidence contract drifted")
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: prior component evidence receipt drifted")
            files = _validate_embedded_files(entry_id, bundle.get("files"), owner="bundle")
            try:
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: natural component result identity drifted")
            ...
            if (
                ...
                raise MemorySourceError(f"{entry_id}: matched component result identity drifted")
        elif evidence_kind == "active-working-set-component-conformance-reproduction":
            if (
                entry_id != "astra-working-set"
                or bundle.get("status") != "ASTRA_WORKING_SET_COMPONENT_CONFORMANCE_PASS"
                or bundle.get("publication_ready") is not False
                or bundle.get("runtime_lane") != "local-arm64-docker-network-none"
                or bundle.get("run_count") != 2
                or bundle.get("claim_boundary")
                ...
                raise MemorySourceError(f"{entry_id}: ASTRA component conformance contract drifted")
            files = _validate_embedded_files(entry_id, bundle.get("files"), owner="bundle")
            try:
            ...
            if (
                ...
                or bundle.get("test_projection_sha256") != verified["projection_sha256"]
                or bundle.get("raw_run_sha256s") != verified["run_sha256s"]
            ):
                raise MemorySourceError(f"{entry_id}: ASTRA component result identity drifted")
...
def validate_source(
    ...
    *,
    ...
    if unknown_layers:
        raise MemorySourceError(f"{entry_id}: unknown memory_layers {sorted(unknown_layers)}")
    ...
    if graph_semantics not in GRAPH_SEMANTICS:
        raise MemorySourceError(f"{entry_id}: unknown graph_semantics {graph_semantics!r}")
    if graph_semantics != "none" and "temporal_graph" not in layers:
        raise MemorySourceError(f"{entry_id}: graph_semantics requires temporal_graph memory layer")
    ...
    if evidence_grade in RECEIPT_GRADES:
        ...
        if not isinstance(reproduction, dict):
            raise MemorySourceError(f"{entry_id}: reproduced source requires reproduction_receipt")
    ...
    if entry_id == "neo4j-agent-memory":
        if not isinstance(cluster_confirmation, dict):
            raise MemorySourceError("neo4j-agent-memory: cluster confirmation receipt is required")
        ...
        if (
            ...
            raise MemorySourceError("neo4j-agent-memory: cluster confirmation receipt is invalid")
        artifact = PROJECT_ROOT / artifact_path
        if not artifact.is_file() or artifact.is_symlink():
            raise MemorySourceError("neo4j-agent-memory: cluster confirmation artifact is missing")
        artifact_bytes = artifact.read_bytes()
        if hashlib.sha256(artifact_bytes).hexdigest() != receipt_sha256:
            raise MemorySourceError("neo4j-agent-memory: cluster confirmation SHA-256 drifted")
        try:
            confirmation_bundle = json.loads(artifact_bytes)
            if not isinstance(confirmation_bundle, dict):
                raise TypeError
            validate_neo4j_h100_evidence(confirmation_bundle, project_root=PROJECT_ROOT)
        ...
        if not isinstance(component_receipt, dict):
            raise MemorySourceError("neo4j-agent-memory: cluster component receipt is required")
        ...
        if (
            ...
            raise MemorySourceError("neo4j-agent-memory: cluster component receipt is invalid")
        component_artifact = PROJECT_ROOT / component_path
        if component_artifact.is_symlink() or not component_artifact.is_file():
            raise MemorySourceError("neo4j-agent-memory: cluster component artifact is missing")
        if hashlib.sha256(component_artifact.read_bytes()).hexdigest() != component_sha256:
            raise MemorySourceError("neo4j-agent-memory: cluster component SHA-256 drifted")
        try:
        ...
        raise MemorySourceError(f"{entry_id}: unexpected cluster_confirmation_receipt")
    elif entry.get("cluster_component_receipt") is not None:
        raise MemorySourceError(f"{entry_id}: unexpected cluster_component_receipt")
    ...
    for index, repository in enumerate(repositories):
        ...
        if not any(
            source_url.rstrip("/") == repository_url or source_url.startswith(f"{repository_url}/")
    ...
    for index, artifact in enumerate(artifacts):
        ...
        if not COMMIT_RE.fullmatch(artifact["revision"]):
            raise MemorySourceError(f"{artifact_id}.revision must be a 40-char immutable revision")
...
def build_reproducibility_audit(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    pinned_repository_sources = sorted(
        entry_id for entry_id, source in sources.items() if source.get("repositories")
    )
    ...
    evidence_grades = Counter(source["evidence_grade"] for source in sources.values())
    ...
    return {
        ...
        "scientific_result_reproduced_source_count": len(scientific_reproduced_sources),
        "scientific_result_reproduced_sources": scientific_reproduced_sources,
        "local_conformance_reproduced_source_count": len(conformance_reproduced_sources),
