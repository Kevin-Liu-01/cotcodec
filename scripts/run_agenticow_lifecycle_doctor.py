#!/usr/bin/env python3
"""Run and seal the exact-source agenticow branch lifecycle falsifier."""
...
from scripts.validate_agenticow_lifecycle_experiment import (  # noqa: E402
    DEFAULT_EXPERIMENT,
    EXPECTED_STATUS,
    validate_experiment_contract,
)

DEFAULT_IMAGE = "cotcodec-agenticow-lifecycle:dd4f437-arm64-v2"
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "data/results/agenticow-branch-lifecycle/2026-08-16-local-docker-v1"
)
DOCKERFILE = PROJECT_ROOT / "infra/memory-baselines/agenticow/Dockerfile"
DOCTOR = PROJECT_ROOT / "infra/memory-baselines/agenticow/doctor.mjs"


class AgenticowRunnerError(RuntimeError):
...
def _write_once(path: Path, data: bytes) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise AgenticowRunnerError(f"short write: {path}")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def _source_contract(source_root: Path, experiment: dict[str, Any]) -> dict[str, Any]:
    ...
    if not source_root.is_dir() or source_root.is_symlink():
        raise AgenticowRunnerError("agenticow source root must be a directory")
    ...
    if head != source["revision"] or tree != source["tree"] or state:
        raise AgenticowRunnerError("agenticow source checkout drifted")
    archive = _run(["git", "-C", str(source_root), "archive", "--format=tar", head]).stdout
    checks = {
        "license_sha256": _sha_path(source_root / "LICENSE"),
        "package_lock_sha256": _sha_path(source_root / "package-lock.json"),
    }
    if _sha(archive) != source["git_archive_tar_sha256"] or checks != {
        ...
        raise AgenticowRunnerError("agenticow source archive or metadata drifted")
    implementation = (source_root / "src/index.js").read_text()
    source_checks = {
        "promotion_has_no_conflict_guard": (
            "target.ingest(flat, ids);" in implementation
            and "promote(target)" in implementation
            and "compare-and-swap" not in implementation
        ),
        "delete_is_tombstone_over_ancestor": (
            "The id stays in" in implementation
            and "the ancestor on disk" in implementation
            and "this._working.tombstones.add(id)" in implementation
        ),
        "manifest_persists_text_for_every_node": (
            "Persisted for EVERY node" in implementation
            and "texts: Object.fromEntries([...n.texts])" in implementation
        ),
    }
    if not all(source_checks.values()):
        raise AgenticowRunnerError("agenticow source-level falsifier drifted")
    return {
        "git_sha": head,
        "git_tree": tree,
        "archive": archive,
...
def _image_contract(image: str, experiment: dict[str, Any]) -> tuple[dict[str, Any], bytes]:
    ...
    if not isinstance(rows, list) or len(rows) != 1 or not isinstance(rows[0], dict):
        raise AgenticowRunnerError("Docker inspect roster drifted")
    inspect = rows[0]
    ...
    if (
        ...
        raise AgenticowRunnerError("agenticow doctor image drifted")
    projection = {
        "image_id": image_id,
        "architecture": "arm64",
        "os": "linux",
...
def _phase_argv(*, image_id: str, volume: str, phase: int, canaries: dict[str, str]) -> list[str]:
    ...
    env_names = {
        "deleted": "COTCODEC_DELETED_CANARY",
        "a": "COTCODEC_A_CANARY",
        "b": "COTCODEC_B_CANARY",
        "nested": "COTCODEC_NESTED_CANARY",
        "child_conflict": "COTCODEC_CHILD_CONFLICT_CANARY",
        "parent_conflict": "COTCODEC_PARENT_CONFLICT_CANARY",
    }
    for key, name in env_names.items():
        argv.extend(["-e", f"{name}={canaries[key]}"])
    argv.append(image_id)
    return argv


def _parse_phase(raw: bytes, expected_phase: int) -> dict[str, Any]:
    marker = b"COTCODEC_AGENTICOW_PHASE="
    rows = [line.split(marker, 1)[1] for line in raw.splitlines() if marker in line]
    if len(rows) != 1:
        raise AgenticowRunnerError(f"phase {expected_phase} emitted {len(rows)} markers")
    payload = json.loads(rows[0])
    if not isinstance(payload, dict) or payload.get("phase") != expected_phase:
        raise AgenticowRunnerError(f"phase {expected_phase} report drifted")
    values = [value for key, value in payload.items() if key != "phase"]
    if not values or not all(value is True for value in values):
        raise AgenticowRunnerError(f"phase {expected_phase} checks failed: {payload}")
    return payload


def _run_repeat(*, repeat: int, image_id: str) -> tuple[dict[str, Any], dict[str, bytes]]:
    volume = f"cotcodec-agenticow-{secrets.token_hex(5)}"
    canaries = {
        key: f"{key}-{secrets.token_hex(16)}"
        for key in ("deleted", "a", "b", "nested", "child_conflict", "parent_conflict")
    }
...
def run(*, source_root: Path, image: str, output: Path) -> dict[str, Any]:
    ...
    if repeats[0]["phases"] != repeats[1]["phases"]:
        raise AgenticowRunnerError("agenticow clean-state repeats diverged")
    findings = {
        key: value
    ...
    if not all(findings.values()):
        raise AgenticowRunnerError(f"agenticow combined checks failed: {findings}")
    summary = {
        "schema_version": 1,
