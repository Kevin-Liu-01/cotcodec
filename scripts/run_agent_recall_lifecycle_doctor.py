#!/usr/bin/env python3
"""Run and seal the exact-source Agent Recall scoped-lifecycle falsifier."""
...
from scripts.validate_agent_recall_lifecycle_experiment import (  # noqa: E402
    DEFAULT_EXPERIMENT,
    EXPECTED_STATUS,
    validate_experiment_contract,
)

DEFAULT_IMAGE = "cotcodec-agent-recall-lifecycle:dcf21b5-arm64-v2"
DEFAULT_OUTPUT = (
    PROJECT_ROOT
    / "data/results/agent-recall-scope-lifecycle/2026-08-16-local-docker-v1"
)
DOCKERFILE = PROJECT_ROOT / "infra/memory-baselines/agent-recall/Dockerfile"
DOCTOR = PROJECT_ROOT / "infra/memory-baselines/agent-recall/doctor.py"


class AgentRecallRunnerError(RuntimeError):
...
def _write_once(path: Path, data: bytes) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise AgentRecallRunnerError(f"short write: {path}")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def _source_contract(source_root: Path, experiment: dict[str, Any]) -> dict[str, Any]:
    ...
    if not source_root.is_dir() or source_root.is_symlink():
        raise AgentRecallRunnerError("Agent Recall source root must be a directory")
    ...
    if head != source["revision"] or tree != source["tree"] or state:
        raise AgentRecallRunnerError("Agent Recall source checkout drifted")
    ...
    if _sha(archive) != source["git_archive_tar_sha256"] or checks != {
        ...
        raise AgentRecallRunnerError("Agent Recall source archive or metadata drifted")
    bridge = (source_root / "agent_recall/mcp_bridge.py").read_text()
    store = (source_root / "agent_recall/store.py").read_text()
    cache = (source_root / "agent_recall/context_gen/cache.py").read_text()
    source_checks = {
        "entity_delete_is_unscoped_row_delete": (
            "self._store.delete_entity(entity_id)" in bridge
            and 'self._conn.execute("DELETE FROM entities WHERE id = ?", (entity_id,))'
            in store
        ),
        "observation_delete_is_soft_archive": (
            '"UPDATE observations SET archived_at = ? "' in store
            and '"WHERE entity_id = ? AND text = ? AND archived_at IS NULL"'
            in store
        ),
        "parent_scope_invalidation_omits_descendants": (
            "Parent: if scope is a hierarchy child, parent is affected" in cache
            and "for parent, children in config.hierarchy.items():" in cache
            and "affected.add(parent)" in cache
        ),
    }
    if not all(source_checks.values()):
        raise AgentRecallRunnerError("Agent Recall source-level falsifier drifted")
    return {
        "git_sha": head,
        "git_tree": tree,
        "archive": archive,
...
def _image_contract(image: str, experiment: dict[str, Any]) -> tuple[dict[str, Any], bytes]:
    ...
    if not isinstance(rows, list) or len(rows) != 1 or not isinstance(rows[0], dict):
        raise AgentRecallRunnerError("Docker inspect roster drifted")
    inspect = rows[0]
    ...
    if (
        ...
        raise AgentRecallRunnerError("Agent Recall doctor image drifted")
    projection = {
        "image_id": image_id,
        "architecture": "arm64",
        "os": "linux",
...
def _phase_argv(
    *, image_id: str, volume: str, phase: int, canaries: dict[str, str]
) -> list[str]:
    return [
        "docker",
        "run",
        ...
        "-e",
        f"COTCODEC_SOFT_CANARY={canaries['soft']}",
        "-e",
        f"COTCODEC_VICTIM_A={canaries['victim_a']}",
        "-e",
        f"COTCODEC_VICTIM_B={canaries['victim_b']}",
        image_id,
    ]


def _parse_phase(raw: bytes, expected_phase: int) -> dict[str, Any]:
    marker = b"COTCODEC_AGENT_RECALL_PHASE="
    ...
    if not isinstance(payload, dict) or payload.get("phase") != expected_phase:
        raise AgentRecallRunnerError(f"phase {expected_phase} report drifted")
    values = [value for key, value in payload.items() if key != "phase"]
    if not values or not all(value is True for value in values):
        raise AgentRecallRunnerError(f"phase {expected_phase} checks failed: {payload}")
    return payload
...
def _run_repeat(
    *, repeat: int, image_id: str
    ...
    volume = f"cotcodec-agent-recall-{suffix}"
    canaries = {
        "soft": f"soft-delete-{secrets.token_hex(16)}",
        "victim_a": f"victim-a-{secrets.token_hex(16)}",
        "victim_b": f"victim-b-{secrets.token_hex(16)}",
    }
...
def run(*, source_root: Path, image: str, output: Path) -> dict[str, Any]:
    ...
    if repeats[0]["phases"] != repeats[1]["phases"]:
        raise AgentRecallRunnerError("Agent Recall clean-state repeats diverged")
    ...
    if not all(findings.values()):
        raise AgentRecallRunnerError(f"Agent Recall combined checks failed: {findings}")
    summary = {
        "schema_version": 1,
