#!/usr/bin/env python3
"""Build and run the exact-source MemForest lifecycle doctor twice."""
...
DEFAULT_SOURCE = PROJECT_ROOT / "raw/baselines/memforest"
DEFAULT_IMAGE = "cotcodec-memforest-lifecycle:fb4320a-arm64-v1"
DEFAULT_OUTPUT = PROJECT_ROOT / "data/results/memforest-lifecycle/2026-08-17-local-docker-v1"
DOCKERFILE = PROJECT_ROOT / "infra/memory-baselines/memforest/Dockerfile"
DOCTOR = PROJECT_ROOT / "infra/memory-baselines/memforest/doctor.py"


class MemForestLifecycleRunnerError(RuntimeError):
    """Raised when source, image, or runtime evidence drifts."""
...
def _write_once(path: Path, data: bytes) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise MemForestLifecycleRunnerError(f"short write: {path}")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def _run(argv: list[str], *, timeout: int = 1200) -> subprocess.CompletedProcess[bytes]:
    ...
    if completed.returncode:
        raise MemForestLifecycleRunnerError(
...
def _source_contract(source_root: Path, experiment: dict[str, Any]) -> dict[str, Any]:
    ...
    if not source_root.is_dir() or source_root.is_symlink():
        raise MemForestLifecycleRunnerError("MemForest source root must be a real directory")
    ...
    if revision != source["revision"] or tree != source["tree"] or status:
        raise MemForestLifecycleRunnerError("MemForest source checkout drifted")
    archive = _run(["git", "-C", str(source_root), "archive", "--format=tar", revision]).stdout
    exact = {name: _sha_path(source_root / name) for name in source["exact_source_files"]}
    static_source_checks = {
        "register_user_joins_unvalidated_user_id": (
            "user_dir=self._snapshot_dir / user_id"
            in (source_root / "src/forest/memforest.py").read_text(encoding="utf-8")
        ),
        "public_tenant_purge_method_absent": all(
            token not in (source_root / "src/forest/memforest.py").read_text(encoding="utf-8")
            for token in ("def delete_user(", "def purge_user(", "def unregister_user(")
        ),
        "save_writes_components_sequentially": all(
            token in (source_root / "src/forest/user_forest.py").read_text(encoding="utf-8")
            for token in (
    ...
    if (
        ...
        or _sha_path(source_root / "requirements.txt") != source["requirements_sha256"]
        or exact != source["exact_source_files"]
        or not all(static_source_checks.values())
    ):
        raise MemForestLifecycleRunnerError("MemForest source receipt drifted")
    return {
        "revision": revision,
        "tree": tree,
        "archive": archive,
...
def _build_image(
    *, source_archive: bytes, source: dict[str, Any], image: str
    ...
    with tempfile.TemporaryDirectory(prefix="cotcodec-memforest-build-") as raw:
        root = Path(raw)
    ...
    if not isinstance(rows, list) or len(rows) != 1 or not isinstance(rows[0], dict):
        raise MemForestLifecycleRunnerError("MemForest image inspection drifted")
    inspect = rows[0]
    config = inspect.get("Config") or {}
    labels = config.get("Labels") or {}
    expected_labels = {
        "org.opencontainers.image.source": "https://github.com/Concyclics/MemForest",
    ...
    if (
        ...
        raise MemForestLifecycleRunnerError("MemForest image provenance drifted")
    projection = {
        "image_id": image_id,
        "architecture": "arm64",
        "os": "linux",
...
def _parse_phase(raw: bytes, expected_phase: int) -> dict[str, Any]:
    ...
    if len(rows) != 1:
        raise MemForestLifecycleRunnerError(
            f"MemForest phase {expected_phase} emitted {len(rows)} markers"
        )
    ...
    if (
        ...
        raise MemForestLifecycleRunnerError(f"MemForest phase {expected_phase} report drifted")
    return payload


def _run_repeat(*, repeat: int, image_id: str) -> tuple[dict[str, Any], dict[str, bytes]]:
    volume = f"cotcodec-memforest-{secrets.token_hex(5)}"
    ...
    try:
        ...
        for phase in (1, 2, 3, 4):
            completed = _run(
                _phase_argv(image_id=image_id, volume=volume, phase=phase, token=token),
                timeout=600,
            )
        ...
        projection = {
            ...
            "stable_projection_sha256": _sha(
                json.dumps(stable_projection, separators=(",", ":"), sort_keys=True).encode()
            ),
        }
...
def run(*, source_root: Path, image: str, output: Path) -> dict[str, Any]:
    ...
    if repeats[0]["stable_projection"] != repeats[1]["stable_projection"]:
        raise MemForestLifecycleRunnerError("MemForest clean-state projections diverged")
    findings = {
        key: value for phase in repeats[0]["phases"] for key, value in phase["checks"].items()
    }
    findings.update(source["static_source_checks"])
    if not all(findings.values()):
        raise MemForestLifecycleRunnerError("MemForest combined findings failed")
    report = {
        "schema_version": 1,
